...
import { loadResources, resetResources, dBattery, getBattery, getFilter, getWater } from './resources.js';
...
// Synchronizace počtu baterií do UI
function syncBatteryDisplay() {
  if (batterySpan) {
    batterySpan.textContent = getBattery();
  }
}
...
if (batteryReplaceBtn) {
  batteryReplaceBtn.addEventListener('click', () => {
    const currentBattery = getBattery();
    if (currentBattery > 0) {
      dBattery(1);
      syncBatteryDisplay();
      hideBatteryReplaceButton();
